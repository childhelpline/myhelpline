...
from helpline.models import cases, hotdesk
...
class AgentTestCase(TestCase):
    ...
    def testFixture(self):
        ...
        self.assertEqual(hotdesk.objects.all().count(), 15)
    ...
    def test_login_page_form_fail(self):
        """Test fail login user"""
        response = self.client.post(reverse('login'),
                                    {'username':self.user.username, 'password': 'password_fail'})
        self.assertTrue(response)
...
class CaseCreateTestCase(TestCase):
    def createCase(self):
        cases.objects.create(hl_key="", hl_user="")
        cases.objects.create(hl_key="", hl_user="")
